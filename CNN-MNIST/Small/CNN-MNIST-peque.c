...
// Fixed convolution with proper weight ordering (for TensorFlow/Keras)
void conv2d_same(const float* input, int in_channels, int in_h, int in_w,
    ...
    // Convolution - TensorFlow order: [kH, kW, inC, outC]
...
int main() {
    ...
    // Allocate memory (using flat arrays for better performance)
    ...
    // ----- LAYER 1: conv2d (4 filters, 3x3, stride 1, same padding) -----
    ...
    // ----- LAYER 2: conv2d_1 (8 filters, 3x3, stride 2, same padding) -----
    ...
    // ----- LAYER 3: conv2d_2 (8 filters, 3x3, stride 1, same padding) -----
    ...
    // ----- LAYER 4: conv2d_3 (12 filters, 3x3, stride 2, same padding) -----
    ...
    // Weight matrix is 12x10 (GAP_OUT x DENSE_OUT), stored row-major: [input][output]
    for (int o = 0; o < DENSE_OUT; o++) {
    ...
    for (int y = 0; y < 28; y++) {
        for (int x = 0; x < 28; x++) {
            ...
            img_sum += val;
            if (val > 0.1) non_zero++;  // Count pixels with significant value
        }
    }
