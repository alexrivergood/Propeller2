 ...
 * Window: 0.25 s  (62 samples @ 250 Hz)
 *
 * Architecture parameters
 * =======================
 *   N_CH     = 6       EEG channels
 *   N_SAMP   = 62      samples per trial
 *   F1       = 8       temporal conv filters
 *   F2       = 16      depthwise / sep output filters
 *   D        = 2       depth multiplier
 *   K_LEN    = 31      Conv2D temporal kernel  (= N_SAMP // 2)
 *   SEP_K    = 15      SepConv kernel          (= min(16, POOL1_T) = 15)
 *                      NOTE: NOT 16 — after Pool1 we have only 15 time steps,
 *                      so a kernel of 16 would extend beyond the signal.
 *   POOL1_T  = 15      time steps after AvgPool1 (= N_SAMP // 4)
 *   POOL2_T  = 1       time steps after AvgPool2 (= POOL1_T // 8)
 *   FLAT_N   = 16      dense input size        (= POOL2_T * F2)
 *
 * Hub RAM budget (static globals)
 * ================================
 *   c1_buf  [6][62][8]  = 2976 floats =  11.6 KB
 *   dw_buf  [1][62][16] =  992 floats =   3.9 KB
 *   p1_buf  [1][15][16] =  240 floats =   0.9 KB
 *   sc_buf  [1][15][16] =  240 floats =   0.9 KB
 *   p2_buf  [1][1 ][16] =   16 floats =  <0.1 KB
 *   flat_buf[16]                        <0.1 KB
 *   Total ≈ 18 KB — smallest of all three variants
 *
 * Compilation:
 *   flexspin --p2 -O2 -o eegnet_seq_0p25s.bin secuencial_0p25s.c
 *
 * Required headers in the same directory:
 *   eegnet_weights_0p25s.h   (from EEGNet_0p25s_train.py)
 *   sample_0p25s.h           (from export_sample_0p25s.py)
 */
...
#include "eegnet_weights_0p25s.h"
#include "sample_0p25s.h"   /* const float input_sample[N_CH][N_SAMP] */

/* ================================================================
   Tensor dimensions
   ================================================================ */
#define N_CH     EEG_N_CHANNELS    /* 6  */
#define N_SAMP   EEG_N_SAMPLES     /* 62 */
#define N_CLS    EEG_N_CLASSES     /* 2  */
#define F1       EEG_F1            /* 8  */
#define F2       EEG_F2            /* 16 */
#define D        EEG_D             /* 2  */
#define K_LEN    EEG_KERNEL_LEN    /* 31 */
#define SEP_K    EEG_SEP_K         /* 15 — NOT 16; see architecture note above */
#define POOL1_T  EEG_POOL1_T       /* 15 */
#define POOL2_T  EEG_POOL2_T       /* 1  */
#define FLAT_N   EEG_FLATTEN_SIZE  /* 16 */
#define BN_EPS   1e-3f
...
/* ================================================================
   Layer 1 – Conv2D temporal  (1 × K_LEN=31, same-pad, no bias)
...
/* ================================================================
   ...
   Input : dw_buf[1][N_SAMP=62][F2]
   Output: p1_buf[1][POOL1_T=15][F2]
   Note: 62 / 4 = 15.5 → floor = 15  (Keras default, last 2 samples unused)
   ================================================================ */
...
/* ================================================================
   ...
   Input : sc_buf[1][POOL1_T=15][F2]
   Output: p2_buf[1][POOL2_T=1 ][F2]
   Note: 15 / 8 = 1.875 → floor = 1  (Keras default, only t=0 used)
   ================================================================ */
static void layer_avgpool2(void)
{
    for (int t = 0; t < POOL2_T; t++) {        /* t = 0 only */
        for (int k = 0; k < F2; k++) {
            float s = 0.0f;
            for (int p = 0; p < 8; p++)
                s += sc_buf[0][t * 8 + p][k];
...
/* ================================================================
   Layer 12 – Dense (16 → 2) + numerically-stable Softmax
...
int main(void)
{
    ...
    printf("Window  : 0.25 s  (%d samples @ 250 Hz)\n", N_SAMP);
