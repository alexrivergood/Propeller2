...
typedef struct {
    ...
    volatile float p2  [1    ][POOL2_T][F2];   /* [1][1][16] */
...
/* Phase 5: AvgPool1 (factor 4) — slice over output time step t
   POOL1_T=15; with 7 workers: workers 0-6 get 2 or 3 steps each. */
...
/* Phase 6: SeparableConv2D — slice over output channel oc
   SEP_K=15, half=7.  Depthwise + pointwise fused per output channel. */
...
/* Phase 8: AvgPool2 (factor 8) — worker 0 only (POOL2_T=1)
   Accesses sc[0][0..7][k] → produces p2[0][0][k]. */
static void ph8_avgpool2(SharedNN *nn, int start, int count)
{
    for (int j = 0; j < count; j++) {
        int t = start + j;   /* t = 0 only */
        for (int k = 0; k < F2; k++) {
            float s = 0.0f;
            for (int p = 0; p < 8; p++)
                s += nn->sc[0][t * 8 + p][k];
