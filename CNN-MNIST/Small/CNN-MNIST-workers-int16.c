...
#include <math.h>
#include <string.h>
#include "mnist_weights_int16.h"  // Changed from mnist_weights_int8.h
...
// Dequantize int16 weight: weight_int16 * scale
static inline float dequantize_int16(int16_t weight, float scale) {
    return (float)weight * scale;
}

// Worker cog - performs parallel computation with INT16 weights
void cog_worker(void *p) {
    ...
    while (1) {
        ...
        // ----- PHASE 1: Conv1 + ReLU (with INT16 weights) -----
        if (phase == 1) {
                        ...
                        for (int ky = 0; ky < 3; ++ky) {
                            for (int kx = 0; kx < 3; ++kx) {
                                ...
                                if (iy >= 0 && iy < IN_H && ix >= 0 && ix < IN_W) {
                                    ...
                                    // Dequantize INT16 weight
                                    float weight = dequantize_int16(conv2d_weights[w_idx], conv2d_scale);
        ...
        // ----- PHASE 2: Conv2 + ReLU (with INT16 weights) -----
        if (phase == 2) {
                            ...
                            for (int kx = 0; kx < 3; ++kx) {
                                ...
                                if (iy >= 0 && iy < C1_H && ix >= 0 && ix < C1_W) {
                                    for (int ic = 0; ic < C1_OUT; ++ic) {
                                        ...
                                        // Dequantize INT16 weight
                                        float weight = dequantize_int16(conv2d_1_weights[w_idx], conv2d_1_scale);
        ...
        // ----- PHASE 3: Conv3 + ReLU (with INT16 weights) -----
        if (phase == 3) {
                            ...
                            for (int kx = 0; kx < 3; ++kx) {
                                ...
                                if (iy >= 0 && iy < C2_H && ix >= 0 && ix < C2_W) {
                                    for (int ic = 0; ic < C2_OUT; ++ic) {
                                        ...
                                        // Dequantize INT16 weight
                                        float weight = dequantize_int16(conv2d_2_weights[w_idx], conv2d_2_scale);
        ...
        // ----- PHASE 4: Conv4 + ReLU (with INT16 weights) -----
        if (phase == 4) {
                            ...
                            for (int kx = 0; kx < 3; ++kx) {
                                ...
                                if (iy >= 0 && iy < C3_H && ix >= 0 && ix < C3_W) {
                                    for (int ic = 0; ic < C3_OUT; ++ic) {
                                        ...
                                        // Dequantize INT16 weight
                                        float weight = dequantize_int16(conv2d_3_weights[w_idx], conv2d_3_scale);
...
int main() {
    ...
    printf("Quantized size (INT16): %.1f KB\n", QUANTIZED_SIZE_KB);
    printf("Compression ratio: %.1fx\n", COMPRESSION_RATIO);
    printf("Quantization: INT%d-bit (Higher Precision)\n", QUANTIZATION_BITS);
    ...
    // ----- Dense Layer (Main cog) with INT16 weights -----
    for (int o = 0; o < DENSE_OUT; ++o) {
        ...
        for (int i = 0; i < GAP_OUT; ++i) {
            ...
            // Dequantize INT16 weight
            float weight = dequantize_int16(dense_weights[weight_idx], dense_scale);
            sum += NN.gap[i] * weight;
        }
        NN.out[o] = sum;
    }
    ...
    printf("INT16 Benefits: Higher precision (65,536 levels vs 256 in INT8)\n");
    ...
    printf("\n=== INT16 Quantized Inference Complete ===\n");
    printf("Higher precision quantization with minimal accuracy loss!\n");
